...
function leerMensajes(imap, message) {
    message.on('message', function (msg, seqno) {
        ...
        msg.on('body', function (stream, info) {
            ...
            stream.on('data', function (chunk) {
                ...
                if (info.which !== 'TEXT') {
                    ...
                    let subject = headerObj.subject.toString()
                    email.subject = subject.replace(/"/g,'\'') // Debido al parseo de la cabecera, todos los caracteres "comilla simple" (') se convirtieron en comillas dobles ("), por lo tanto es necesario revertir el proceso para que la palabra d"asignatura vuelva a ser d'asignatura

                    email.from = headerObj.from
                    email.to = headerObj.to
                    console.log("Aux es igual a: "+ email.subject)
                }
            })
            stream.once('end', function () { //['HEADER.FIELDS (FROM)','TEXT']

                if (info.which === 'TEXT') {
                    if(email.subject == "Modificació professor/a grup d'asignatura de teleeducació.")
        ...
        msg.once('end', function () {
            ...
            return JSON.stringify(email) //Devolvemos el email en formato JSON
        });
    });
